...
rolling_ewm = rolling_ewm[(rolling_ewm.index >= '2014-01-02')] 
rolling_returns = rolling_returns[(rolling_returns.index >= '2014-01-02')] 
...
def momentum_strategy_positions(rolling_ewm, returns):
    ...
    positions = []
    i = 0
    while i < len(rolling_ewm):
        if rolling_ewm[i] > returns[i]:  # Sell if todays return is lower than ewm
            positions.append(-1) 
            i += 1
            
        elif rolling_ewm[i] < returns[i]:  # Buy if todays return is higher than ewm
            positions.append(+1)  # 
            i += 1
...
positions = positions[positions.index>='2014-01-02']
...
def compute_win_loss_probability(excess_returns):
    ...
    negative_returns = np.sum(np.array(excess_returns) < 0)
...
theta_train = theta[theta.index<="2023-1-1"]
theta_test = theta[theta.index>"2023-1-1"]
...
def daily_trading_pnl(theta,excess_daily_returns):
    ...
    for i in range(len(theta)):
     transaction_cost[i] = np.abs(theta[i]-theta[i-1]) * 0.0075
...
NVIDIA_excess_returns_train = NVIDIA_excess_returns[NVIDIA_excess_returns.index <= '2023-1-1'] 
NVIDIA_excess_returns_test =  NVIDIA_excess_returns[NVIDIA_excess_returns.index > '2023-1-1'] 
...
cumulated_pnl_dv_train = cumulated_pnl_dv[cumulated_pnl_dv.index <= '2023-1-1' ]
cumulated_pnl_dv_test = cumulated_pnl_dv[cumulated_pnl_dv.index > '2023-1-1' ]
...
daily_total_train = daily_total_pnl[daily_total_pnl.index <= '2023-1-1']
daily_total_test = daily_total_pnl[daily_total_pnl.index >'2023-1-1']
daily_total_pnl_cum = daily_total_pnl.cumsum()
daily_total_cum_train = daily_total_pnl_cum[daily_total_pnl_cum.index <= '2023-1-1']
daily_total_cum_test = daily_total_pnl_cum[daily_total_pnl_cum.index > '2023-1-1']
daily_cap_pnl = daily_total_pnl - daily_pnl
daily_cap_train = daily_cap_pnl[daily_cap_pnl.index <= '2023-1-1']
daily_cap_test = daily_cap_pnl[daily_cap_pnl.index >'2023-1-1']
daily_cap_pnl_cum = daily_cap_pnl.cumsum()
daily_cap_cum_train = daily_cap_pnl_cum[daily_cap_pnl_cum.index <= '2023-1-1']
daily_cap_cum_test = daily_cap_pnl_cum[daily_cap_pnl_cum.index > '2023-1-1']
...
port_excess_returns_train = portfolio_excess_returns[portfolio_excess_returns.index <= '2023-1-1']
port_excess_returns_test = portfolio_excess_returns[portfolio_excess_returns.index > '2023-1-1']
...
rolling_sharpe_train = rolling_sharpe[rolling_sharpe.index <= '2023-1-1']
rolling_sharpe_test = rolling_sharpe[rolling_sharpe.index > '2023-1-1']
...
rolling_VaR.index = daily_cap_pnl.index
rolling_ES.index = daily_cap_pnl.index
rolling_VaR_mc.index = daily_cap_pnl.index
